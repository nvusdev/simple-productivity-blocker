...
def test_dns_matcher():
    ...
    assert not matcher.matches("www.facebook.com")
...
def test_hosts_fallback_hierarchy():
    ...
    resolved = _resolve_hosts_fallback_domains(
        manual_domains=set(),
        normalized_filter_domains={"ads.google.com", "tracker.com"},
    ...
    # 3. Exceptions > Content Filter (Filtered before passing, as in daemon.py)
    exceptions = {"ads.google.com"}
    raw_filters = {"ads.google.com", "tracker.com"}
    filtered_filters = {d for d in raw_filters if d not in exceptions}
    resolved = _resolve_hosts_fallback_domains(
        manual_domains=set(),
    ...
    # 4. Manual Block > Exceptions
    exceptions = {"ads.google.com"}
    raw_filters = {"ads.google.com"}
    filtered_filters = {d for d in raw_filters if d not in exceptions}
    resolved = _resolve_hosts_fallback_domains(
